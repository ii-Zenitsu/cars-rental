...
function Header() {
  ...
  return (
    <div className="navbar bg-base-100 mb-4 md:w-full">
      <div className="navbar-start">
        <Link className="btn btn-primary border-2 text-xl font-bold rounded-3xl hidden md:inline-flex" to="/">Med Cars</Link>
      </div>
      <div className="navbar-center flex">
        <ul className="menu menu-horizontal gap-3 px-0 py-1 mx-2 relative">
          {user && user.role !== "admin" && (
            <>
              <li ref={(e) => (tab.current["/"] = e)}><NavLink className={({ isActive }) => `btn btn-sm transition-[border-radius] duration-500 rounded-2xl md:hidden btn-outline btn-accent ${isActive ? "rounded-b-none font-bold text-base" : ""}`} to="/">Home</NavLink></li>
      ...
      <div className="navbar-end">
        {user ? (
          <>
          <button className="btn btn-sm btn-outline btn-accent rounded-2xl mx-2 hidden md:inline-flex" onClick={logout}>Logout<LogOut size={16} /></button>
          <button className="btn btn-sm btn-outline btn-accent rounded-2xl mx-2 md:hidden" onClick={logout}><LogOut size={16} /></button>
          </>
        ) : (
          <>
          <NavLink className={({ isActive }) => `btn btn-sm btn-outline btn-accent hidden md:inline-flex transition-colors duration-500 rounded-2xl mx-2 ${isActive ? "btn-active font-bold text-base" : ""}`} to="/sign"><LogIn size={16} />Sign In</NavLink>
          <NavLink className={({ isActive }) => `btn btn-sm btn-outline btn-accent md:hidden transition-colors duration-500 rounded-2xl mx-2 ${isActive ? "btn-active font-bold text-base" : ""}`} to="/sign"><LogIn size={16} /></NavLink>
          </>
        )}
      </div>
    </div>
